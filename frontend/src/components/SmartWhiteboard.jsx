...
import { Activity, Cpu, Sliders, Waves, Layers } from 'lucide-react';

export default function SmartWhiteboard({ visualInstruction }) {
  const [sliderR, setSliderR] = useState(4);
  const [sliderV, setSliderV] = useState(12);

  // Sync initial values from instruction if provided
  useEffect(() => {
  ...
  if (!visualInstruction) {
    return (
      <div className="glass-panel whiteboard-surface" style={{ height: '360px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <p style={{ color: 'var(--text-muted)' }}>Whiteboard ready for concept presentation.</p>
      </div>
    );
  }

  const { type, title, caption, data } = visualInstruction;

  // Render Math KaTeX helper
  ...
  return (
    <div className="glass-panel" style={{ padding: '20px', display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '10px' }}>
        ...
        <span className="badge badge-indigo">{type} visual</span>
      </div>

      {/* Main Visual Canvas Area */}
      <div className="whiteboard-surface" style={{ flex: 1, minHeight: '300px', padding: '20px', position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
        
        {/* CASE 1: Interactive Circuit Simulation */}
        {type === 'circuit' && (
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
            {(() => {
              ...
              return (
                <>
                  {/* Circuit SVG */}
                  <svg viewBox="0 0 460 210" style={{ width: '100%', maxWidth: '440px', height: '190px' }}>
                    <defs>
                    ...
                    {/* Circuit Wire Loop */}
                    <rect x="50" y="30" width="360" height="150" rx="14" fill="none" stroke="#334155" strokeWidth="4" />
                    
                    {/* Animated Electron Current Flow */}
                    <rect 
                      x="50" y="30" width="360" height="150" rx="14" 
                      fill="none" 
                      stroke="#06B6D4" 
                      strokeWidth="3" 
                    ...
                    {/* Formula Banner in Center */}
                    <g transform="translate(230, 115)">
                      <rect x="-80" y="-18" width="160" height="36" rx="8" fill="rgba(15, 23, 42, 0.9)" stroke="rgba(255,255,255,0.1)" />
                      <text x="0" y="6" textAnchor="middle" fill="#F8FAFC" fontSize="13" fontWeight="600" fontFamily="var(--font-mono)">
                  ...
                  <div style={{ width: '100%', maxWidth: '440px', background: 'rgba(255,255,255,0.04)', padding: '12px 16px', borderRadius: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                      <span>1Ω (High Current: {sliderV}A)</span>
                      <span>20Ω (Low Current: {(sliderV/20).toFixed(2)}A)</span>
                    </div>
                  </div>
        ...
        {/* CASE 2: Mathematical Derivations */}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', width: '100%', maxWidth: '420px' }}>
            {data?.steps?.map((step, idx) => (
              <div 
                key={idx} 
                style={{
                  background: idx === (data.active_step ?? 0) ? 'rgba(99, 102, 241, 0.15)' : 'rgba(255, 255, 255, 0.03)',
              ...
              >
                <div>
                  <div style={{ fontSize: '1.2rem', marginBottom: '4px' }} dangerouslySetInnerHTML={renderMath(step.latex)} />
                  <span style={{ fontSize: '0.78rem', color: 'var(--text-secondary)' }}>{step.explanation}</span>
                </div>
                {idx === (data.active_step ?? 0) && <span className="badge badge-indigo">Focus</span>}
              </div>
            ))}
          </div>
        )}
        ...
        {type === 'code_trace' && (
          <div style={{ width: '100%', maxWidth: '460px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <pre style={{ background: '#090D16', padding: '14px', borderRadius: '8px', fontSize: '0.85rem', color: '#38BDF8', overflowX: 'auto', border: '1px solid rgba(255,255,255,0.06)' }}>
              <code>{data.code}</code>
            </pre>
            {data.trace_state && (
              <div style={{ display: 'flex', gap: '12px', background: 'rgba(255,255,255,0.04)', padding: '8px 14px', borderRadius: '8px', fontSize: '0.8rem' }}>
                <span><strong>low:</strong> {data.trace_state.low}</span>
                <span><strong>mid:</strong> {data.trace_state.mid}</span>
                <span><strong>high:</strong> {data.trace_state.high}</span>
                <span><strong>target:</strong> {data.trace_state.target}</span>
              </div>
            )}
          </div>
        )}

        {/* CASE 4: General Concept Map */}
        {type === 'concept_map' && (
            ...
            {data?.concepts?.map((c, i) => (
              <div 
                ...
                style={{
                  padding: '10px 14px',
                  borderRadius: '8px',
                  background: c === data.active_concept ? 'rgba(99, 102, 241, 0.2)' : 'rgba(255, 255, 255, 0.04)',
